...
export default function DashboardPage() {
    ...
    const endpoint =
      user?.role === 'Retail Manager'
        ...
        : user?.role === 'Warehouse Operator'
          ? '/dashboard/warehouse'
          : user?.role === 'Food Quality Inspector'
            ? '/dashboard/inspector'
  ...
  const isRetail = role === 'Retail Manager'
  const isWarehouse = role === 'Warehouse Operator'
  const isInspector = role === 'Food Quality Inspector'
  ...
  return (
    <div className="page-container animate-in">
      <div className="page-header">
        ...
        <p>
          {isAdmin
            ...
            : isRetail
              ? 'Retail Freshness Analytics'
              : isWarehouse
                ? 'Warehouse Monitoring'
                : isInspector
                  ? 'Food Quality Inspection Dashboard'
                  : 'Your Food Freshness Overview'}
        </p>
      </div>
      ...
      {isInspector && data && <InspectorView data={data} />}
      ...
      <div className="glass-card">
        ...
        {data.recent_analyses?.length ? (
              ...
              <tbody>
                {data.recent_analyses.map(a => (
                  <tr key={a.id}>
                    ...
                    <td>
                      <span
                        style={{
                          fontWeight: 700,
                          color:
                            a.freshness_score >= 70
                              ? 'var(--green)'
                              : a.freshness_score >= 50
                                ? 'var(--yellow)'
                                : 'var(--red)'
                        }}
                      >
                    ...
                    <td>
                      <span
                        className={`badge badge-${a.quality_class
                          .toLowerCase()
                          .replace(' ', '-')}`}
                      >
                        {a.quality_class}
                    ...
                    <td>
                      <span
                        className={`badge badge-${a.risk_level.toLowerCase()}`}
                      >
                        {a.risk_level}
                      </span>
                    </td>
          ...
          <p style={{ color: 'var(--text-muted)' }}>
            No analyses yet. Upload a food image to get started!
          </p>
        )}
      </div>
    </>
...
function InspectorView({ data }) {
  return (
    <>
      <div className="grid-4" style={{ marginBottom: 24 }}>
        <ScoreCard icon="🔬" value={data.total_analyses || 0} label="Inspections" color="accent" />
        <ScoreCard icon="✅" value={data.fresh_items || 0} label="Fresh Items" color="green" />
        <ScoreCard icon="⚠️" value={data.spoiled_items || 0} label="Spoiled Items" color="red" />
        <ScoreCard icon="📊" value={`${data.avg_freshness || 0}%`} label="Avg Freshness" color="blue" />
      </div>
              ...
              <tbody>
                {data.recent_users?.map(u => (
                  <tr key={u.id}>
                    ...
                    <td>
                      <span className="badge badge-good">{u.role}</span>
                    </td>
                    <td
                      style={{
                        color: 'var(--text-secondary)',
                    ...
                    >
                      {new Date(u.date || u.created_at).toLocaleDateString()}
                    </td>
                  </tr>
                ))}
              </tbody>
