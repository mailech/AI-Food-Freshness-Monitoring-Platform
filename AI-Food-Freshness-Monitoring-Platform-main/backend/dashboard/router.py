...
def consumer_dashboard(
    ...
    items = db.query(FoodItem).filter(
        FoodItem.added_by == user.id
    ).count()

    analyses = db.query(AnalysisRecord).filter(
        AnalysisRecord.user_id == user.id
    ).all()

    avg_freshness = (
        sum(a.freshness_score for a in analyses) / len(analyses)
        if analyses else 0
    )
    ...
    fresh_count = sum(
        1 for a in analyses if a.freshness_score >= 70
    )

    spoiled_count = sum(
        1 for a in analyses
        if a.quality_class == "Spoiled"
    )
...
def retail_dashboard(
    user: User = Depends(
        require_role("Retail Manager", "Administrator")
    ),
    db: Session = Depends(get_db)
):
    ...
    expiring = db.query(Batch).filter(
        ...
        Batch.expiry_date <= now + timedelta(days=3)
    ).count()

    daily_analyses = db.query(
        func.date(AnalysisRecord.created_at),
        func.count(AnalysisRecord.id),
        func.avg(AnalysisRecord.freshness_score)
    ).group_by(
        func.date(AnalysisRecord.created_at)
    ).order_by(
        func.date(AnalysisRecord.created_at).desc()
    ).limit(14).all()
...
def warehouse_dashboard(
    user: User = Depends(
        require_role("Warehouse Operator", "Administrator")
    ),
    db: Session = Depends(get_db)
):
    total_items = db.query(FoodItem).count()
    ...
    compliant = sum(
        1 for c in conditions if c.is_compliant
    )

    compliance_rate = (
        compliant / len(conditions) * 100
        if conditions else 100
    )

    avg_temp = (
        sum(c.temperature for c in conditions) / len(conditions)
        if conditions else 0
    )

    avg_humidity = (
        sum(c.humidity for c in conditions) / len(conditions)
        if conditions else 0
    )
...
def inspector_dashboard(
    user: User = Depends(
        require_role("Food Quality Inspector", "Administrator")
    ),
    db: Session = Depends(get_db)
):
    analyses = db.query(AnalysisRecord).all()
    ...
    fresh_items = sum(
        1 for a in analyses
        if a.quality_class in ["Fresh", "Good"]
    )
    ...
    avg_freshness = (
        sum(a.freshness_score for a in analyses)
        / total_analyses
        if analyses else 0
    )
    ...
    for a in analyses:
        if a.quality_class in quality_distribution:
            quality_distribution[a.quality_class] += 1
