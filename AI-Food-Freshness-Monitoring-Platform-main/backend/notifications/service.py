...
def check_and_generate_alerts(db: Session, user_id: int):
    ...
    for analysis in recent_analyses:
        ...
        spoilage_percentage = analysis.spoilage_probability

        if analysis.freshness_score < 30 or spoilage_percentage >= 70:
            ...
            if not existing:
                n = create_notification(
                    ...
                    f"{analysis.food_name} | Spoilage: {spoilage_percentage:.1f}% | Status: Spoiled",
                    "high",
                    analysis.id
                )
                alerts.append(n)
            ...
            if not existing:
                n = create_notification(
                    ...
                    f"{analysis.food_name} | Spoilage: {spoilage_percentage:.1f}% | Status: Near Spoilage",
                    "medium",
                    analysis.id
                )
                alerts.append(n)
